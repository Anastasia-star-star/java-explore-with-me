...
public class EventPublicServiceImpl implements EventPublicService {
    ...
    public List<EventShortDto> getAllEvents(
        ...
        endpHit.setApp("ewm-service");
        endpHit.setUri(request.getRequestURI());
        endpHit.setIp(request.getRemoteAddr());
        endpHit.setTimestamp(LocalDateTime.now());
        statistic.statsClient.saveHit(endpHit);
        ...
        if (events != null) {
            for (Event event : events) {
                endpHit.setUri("/events/" + event.getId());
                endpHit.setTimestamp(LocalDateTime.now());
                statistic.statsClient.saveHit(endpHit);
            }
        }
