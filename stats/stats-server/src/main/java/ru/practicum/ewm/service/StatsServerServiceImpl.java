...
public class StatsServerServiceImpl implements StatsServerService {
    ...
    @Transactional
    @Override
    public EndpHit saveEndpointHit(EndpHit endpHit) {
        try {
            Hit hit = statsServerRepository.save(StatsServerMapper.INSTANCE.toHit(endpHit));
            return StatsServerMapper.INSTANCE.toEndpointHit(hit);
        } catch (DataIntegrityViolationException e) {
            throw new HitDontSaveException("Информация о запросе не была сохранена: " + endpHit);
        }
    }
