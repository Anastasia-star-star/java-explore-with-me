...
class StatsServerServiceImplTest {
    ...
    void saveEndpointHit_whenEndpointHitValid_thenSavedEndpointHit() {
        EndpHit endpHitToSave = new EndpHit();
        endpHitToSave.setIp("1.1.1.1");
        when(statsServerRepository.save(any(Hit.class)))
                .thenReturn(StatsServerMapper.INSTANCE.toHit(endpHitToSave));

        EndpHit actualEndpHit = statsServerService.saveEndpointHit(endpHitToSave);

        assertThat(endpHitToSave, equalTo(actualEndpHit));
    ...
    void saveEndpointHit_whenEndpointHitNotValid_thenExceptionThrown() {
        ...
        final HitDontSaveException exception = assertThrows(HitDontSaveException.class,
                () -> statsServerService.saveEndpointHit(endpHitToSave));

        assertThat("Информация о запросе не была сохранена: " + endpHitToSave,
